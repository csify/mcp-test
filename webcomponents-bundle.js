...
`;var o=class extends HTMLElement{static get observedAttributes(){return["size","interaction-state","semantic-state","icon-src","label"]}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.appendChild(n.content.cloneNode(!0))}connectedCallback(){this.update()}attributeChangedCallback(){this.update()}update(){let a=this.shadowRoot.querySelector("button"),e=this.shadowRoot.querySelector(".fab-icon"),t=this.shadowRoot.querySelector(".fab-label");a.className=["fab-root",`fab-${this.getAttribute("size")||"lg"}`,`fab-${(this.getAttribute("interaction-state")||"default").toLowerCase()}`,`fab-sem-${(this.getAttribute("semantic-state")||"default").toLowerCase()}`].join(" "),this.getAttribute("icon-src")?e.innerHTML=`<img src="${this.getAttribute("icon-src")}" alt="FAB Icon" style="width: 24px; height: 24px; object-fit: contain;" />`:e.innerHTML="",this.getAttribute("label")?(t.textContent=this.getAttribute("label"),t.style.display=""):(t.textContent="",t.style.display="none")}};customElements.define("mds-fab",o);
