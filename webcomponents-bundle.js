...
`;var r=class extends HTMLElement{static get observedAttributes(){return["type","size","interaction-state","semantic-state","has-icon","has-label","has-menu","label","icon-src"]}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.appendChild(s.content.cloneNode(!0))}connectedCallback(){this.update()}attributeChangedCallback(){this.update()}update(){let o=this.shadowRoot.querySelector("button"),t=this.shadowRoot.querySelector(".btn-icon"),e=this.shadowRoot.querySelector(".btn-label");if(o.className=["btn",`btn-${this.getAttribute("type")||"primary"}`,`btn-${this.getAttribute("size")||"lg"}`,`btn-${(this.getAttribute("interaction-state")||"default").toLowerCase()}`,`btn-sem-${(this.getAttribute("semantic-state")||"default").toLowerCase()}`].join(" "),this.hasAttribute("has-icon")&&this.getAttribute("icon-src")){let i=getComputedStyle(o).color;t.innerHTML=`<img src="${this.getAttribute("icon-src")}" alt="icon" style="width: 20px; height: 20px; object-fit: contain; filter: none; color: ${i};" />`,t.style.display=""}else t.innerHTML="",t.style.display="none";this.hasAttribute("has-label")?(e.textContent=this.getAttribute("label")||"A button's text",e.style.display=""):(e.textContent="",e.style.display="none")}};customElements.define("mds-button",r);var n=document.createElement("template");n.innerHTML=`
  <style>
...
`;var a=class extends HTMLElement{static get observedAttributes(){return["size","interaction-state","semantic-state","icon-src","label"]}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.appendChild(n.content.cloneNode(!0))}connectedCallback(){this.update()}attributeChangedCallback(){this.update()}update(){let o=this.shadowRoot.querySelector("button"),t=this.shadowRoot.querySelector(".fab-icon");if(o.className=["fab-root",`fab-${this.getAttribute("size")||"lg"}`,`fab-${(this.getAttribute("interaction-state")||"default").toLowerCase()}`,`fab-sem-${(this.getAttribute("semantic-state")||"default").toLowerCase()}`].join(" "),this.getAttribute("icon-src")){let e=getComputedStyle(o).color;t.innerHTML=`<img src="${this.getAttribute("icon-src")}" alt="FAB Icon" style="width: 24px; height: 24px; object-fit: contain; filter: none; color: ${e};" />`}else t.innerHTML=""}};customElements.define("mds-fab",a);
